...
func (ctx *context) printDiff(a, b interface{}) {
	...
	case reflect.String:
		...
		case string:
			bb, ok := b.(string)
			if !ok || aa != bb {
				if len(aa) > 64 || len(bb) > 64 {
					// NOTE GO-WEB-APP PATCHED
					ctx.tag(&ctx.opts.Changed)
					ctx.buf.WriteString(dmp.DiffPrettyHtml(dmp.DiffMain(aa, bb, false)))
					// NOTE GO-WEB-APP PATCHED
				} else {
					ctx.printMismatch(a, b)
				}
...
func Compare(a, b []byte, opts *Options) (Difference, string) {
	...
	ctx := context{opts: opts, buf: *bytes.NewBuffer(make([]byte, 1024))}
	ctx.printDiff(av, bv)
	if ctx.lastTag != nil {
		ctx.buf.WriteString(ctx.lastTag.End)
	}
